...
# df_scaled = ... # Your preprocessed DataFrame

# Assuming models is a dictionary containing model names and their corresponding cluster labels
# models = {'KMeans': kmeans_labels, 'GaussianMixture': gmm_labels, 'Hierarchical': hierarchical_labels, ...}

def align_clusters(reference_labels, target_labels):
    """
    Align target_labels to match reference_labels using the Hungarian algorithm.
    """
    reference_labels = np.array(reference_labels)
    target_labels = np.array(target_labels)

    # Create a contingency matrix (cost matrix)
    contingency_matrix = np.zeros((len(np.unique(reference_labels)), len(np.unique(target_labels))))

    for i, ref_label in enumerate(np.unique(reference_labels)):
        for j, target_label in enumerate(np.unique(target_labels)):
            contingency_matrix[i, j] = np.sum((reference_labels == ref_label) & (target_labels == target_label))

    # Apply Hungarian algorithm
    row_ind, col_ind = linear_sum_assignment(-contingency_matrix)  # Use negative for maximization

    # Create a mapping from target to reference
    mapping = {col_ind[i]: row_ind[i] for i in range(len(row_ind))}
    new_target_labels = np.array([mapping[label] for label in target_labels])

    return new_target_labels

# Reference model for alignment (e.g., KMeans)
reference_model = 'KMeans'
reference_labels = models[reference_model]

# Align other models to the reference model
aligned_models = {}
for model_name, labels in models.items():
    if model_name != reference_model:
        aligned_labels = align_clusters(reference_labels, labels)
        aligned_models[model_name] = aligned_labels
    else:
        aligned_models[model_name] = labels

# Now you can use aligned_models for further analysis and plotting

# Create a function to evaluate clustering
def evaluate_clustering(X, labels):
    if len(set(labels)) > 1:  # Ignore methods that did not find any clusters
...
class CustomClusterEnsemble(BaseEstimator, ClusterMixin):
    ...
    def predict(self, X):
        kmeans_labels = self.kmeans.predict(X)
        hierarchical_labels = self.hierarchical.fit_predict(X)
        gmm_labels = self.gmm.predict(X)

        labels = np.vstack((kmeans_labels, hierarchical_labels, gmm_labels)).T
        final_labels = np.apply_along_axis(lambda x: np.argmax(np.bincount(x)), axis=1, arr=labels)
...
# Define the scoring function (silhouette score)
scoring = make_scorer(silhouette_score, greater_is_better=True)

# Create an instance of the custom clustering ensemble
cluster_ensemble = CustomClusterEnsemble()

# Perform grid search
grid_search = GridSearchCV(cluster_ensemble, param_grid, cv=5, scoring=scoring)
...
# Use the best hyperparameters to create the optimized clustering ensemble
optimized_ensemble = CustomClusterEnsemble(**best_params)
optimized_ensemble.fit(df_scaled)
optimized_ensemble_labels = optimized_ensemble.predict(df_scaled)

# Evaluate the optimized ensemble
silhouette, calinski, davies_bouldin = optimized_ensemble.score(df_scaled, optimized_ensemble_labels)
...
# Plot the clustering results of the optimized ensemble
plot_clusters(df_scaled, optimized_ensemble_labels)
...
class WeightedClusterEnsemble(BaseEstimator, ClusterMixin):
    def __init__(self, n_clusters=3):
        self.n_clusters = n_clusters
        self.kmeans = KMeans(n_clusters=self.n_clusters)
        self.hierarchical = AgglomerativeClustering(n_clusters=self.n_clusters)
    ...
    def fit(self, X, y=None):
        ...
        # Assign weights based on silhouette scores
        self.weights = np.array([kmeans_silhouette, hierarchical_silhouette, gmm_silhouette])
    ...
    def predict(self, X):
        # Use precomputed labels and weights to combine predictions
        labels = np.vstack((self.kmeans_labels, self.hierarchical_labels, self.gmm_labels)).T
        weighted_labels = np.apply_along_axis(lambda x: np.argmax(np.bincount(x, weights=self.weights)), axis=1, arr=labels)
...
# Create an instance of the custom clustering ensemble
cluster_ensemble = WeightedClusterEnsemble()
...
# Use the best hyperparameters to create the optimized clustering ensemble
optimized_ensemble = WeightedClusterEnsemble(**best_params)
optimized_ensemble.fit(df_scaled)
optimized_ensemble_labels = optimized_ensemble.predict(df_scaled)
...
# param_grid = {
#     'encoding_dim': [1, 2, 3, 4, 5],  # Thử nghiệm các giá trị cho chiều mã hóa
...
# estimator = KerasRegressor(build_fn=create_autoencoder)
...
- batch_size=32: Đây là số lượng mẫu dữ liệu sẽ được sử dụng trong mỗi lần cập nhật trọng số của mạng. Cụ thể, mỗi lần mạng sẽ nhận vào 32 mẫu dữ liệu và cập nhật trọng số dựa trên hiệu suất của chúng. Việc sử dụng batch giúp tăng tốc độ huấn luyện và giảm bộ nhớ cần thiết.
